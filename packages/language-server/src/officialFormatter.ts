...
// Keep this file synchronized with the upstream formatter contract.
...
type Token = { text: string; kind: "word" | "number" | "literal" | "symbol"; gap: boolean };
type Line = { indent: string; code: string; comment: string; tokens: Token[] };
...
function splitLine(text: string): Line {
  ...
  for (let i = 0; i < source.length; i += 1) {
    ...
    else if (char === "#") { commentAt = i; break; }
  }
...
function lex(code: string): Token[] {
  ...
  let hadGap = false;
  while (i < code.length) {
    if (/\s/.test(code[i])) { hadGap = true; i += 1; continue; }
    const start = i;
    const char = code[i];
    ...
    if (char === '"' || char === "'") {
      ...
      if (/[eE]/.test(code[i] ?? "") && (DIGIT.test(code[i + 1] ?? "") || (/[+-]/.test(code[i + 1] ?? "") && DIGIT.test(code[i + 2] ?? "")))) {
        i += 1;
        if (/[+-]/.test(code[i] ?? "")) i += 1;
        while (i < code.length && DIGIT.test(code[i])) i += 1;
      }
...
function indentDepths(lines: Line[]): number[] {
  ...
  for (const line of lines) {
    if (line.code === "" && line.comment === "") { depths.push(-1); continue; }
...
export function formatBend(source: string, options: FormatOptions = {}): string {
  ...
  let lines: Line[];
  try { lines = rawLines.map(splitLine); } catch { return source; }
  const depths = indentDepths(lines);
  ...
  try {
    const checkLines = formatted.replace(/\r\n/g, "\n").split("\n").slice(0, finalEol ? -1 : undefined).map(splitLine);
    if (fingerprint(lines) !== fingerprint(checkLines)) return source;
  } catch { return source; }
  return formatted;
}
